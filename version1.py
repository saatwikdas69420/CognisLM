...
# Expanded Vocabulary and Word Encoding
words = [
    ...
    "feelings", "emotions", "speed", "power", "altitude", "happiness", "winning", "losing", "control", "wins", "sadness", "victory", "failure", "strength", "darkness", "destruction", "creation", ""
]
...
class SimpleLanguageModel:
    def __init__(self, vocab_size, hidden_size, context_size=2):
    ...
    def predict(self, input_sentence, temperature=1.0):
...
context_size = 3  # Allow for more context in predictions
...
while True:
    ...
    # Start with the user's input and generate the next word
    generated_text = input_sentence
    for _ in range(5):  # Generate 5 words
        next_word = model.predict(" ".join(context), temperature=1.2)  # Increased temperature for randomness
        print(f"{generated_text} -> {next_word}")
        generated_text = generated_text + " " + next_word  # Update the input sentence for the next prediction
        context.append(next_word)  # Add the new word to the context
